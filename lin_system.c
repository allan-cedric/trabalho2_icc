    ...
    for (int i = 0; i < ls->U.n; i++)
    {
        ...
        for (int k = i + 1; k < ls->U.n; k++)
        {
            ...
            for (int j = i + 1; j < end_step; j += UNROLL_STEP)
            {
                ...
                ls->U.A[kj] -= (ls->U.A[ij] * mp);         // U(k, j) -= U(i, j) * mp
                ls->U.A[kj + 1] -= (ls->U.A[ij + 1] * mp); // U(k, j + 1) -= U(i, j + 1) * mp
                ls->U.A[kj + 2] -= (ls->U.A[ij + 2] * mp); // U(k, j + 2) -= U(i, j + 2) * mp
                ls->U.A[kj + 3] -= (ls->U.A[ij + 3] * mp); // U(k, j + 3) -= U(i, j + 3) * mp
            }
            // Resíduo
...
{
    // Verifica se existe uma fatoração LU
    if (!ls->L.A || !ls->U.A)
    {
    ...
    real_t *x = (real_t *)malloc(ls->n * sizeof(real_t)); // Vetor solução alocado
    alloc_test(x);

    real_t *y = (real_t *)malloc(ls->n * sizeof(real_t)); // Vetor b escalonado alocado
    alloc_test(y);

    real_t *b = (real_t *)malloc(ls->n * sizeof(real_t)); // Cópia do vetor b original alocado
    alloc_test(b);
